...
class SearchImagesCommandTest : FlickrImageCommandTestBase() {
    private lateinit var command: SearchImagesCommand

    override fun setup() {
        super.setup()
        command = spy(SearchImagesCommand(apiKey, "tomato", numImagesPerPage, pageNum, mockOnSuccess, mockOnFailure))
    ...
    fun `given command run, api should get request for images`() {
        ...
        /** Verify **/
        verify(mockFlickrApi).searchPhotos(any(), any(), any(), any(), anyOrNull())
    }
